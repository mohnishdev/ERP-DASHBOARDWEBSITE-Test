...
export function CRMLeads() {
  ...
  const convertLead = (lead: Lead) => {
    if (dashboardDB.customers.includes(lead.company)) {
      showToast(`${lead.company} is already a customer`);
      return;
    }
    dashboardDB.customers.unshift(lead.company);
    lead.status = "Won";
    setLeads([...dashboardDB.leads]);
    closeModal();
  ...
  return <>
    ...
    {activeTab === "pipeline" && <div className="kanban">{lanes.map((lane) => { const items = leads.filter((lead) => lane.matches(lead.status)); return <div className="kcol" style={{ "--kcolor": lane.color } as React.CSSProperties} key={lane.label}><div className="kcol-head"><h4>{lane.label}</h4><span className="badge" style={{ background: `${lane.color}22`, color: lane.color }}>{items.length}</span></div>{items.length ? items.map((lead) => <div className="kcard" onClick={() => openLeadDetail(lead)} key={lead.id}><div className="kcard-top"><span className="kcard-name">{lead.company}</span>{priorityBadge(lead.priority)}</div><div className="kcard-meta">{lead.contactName} · {fmtNaira(lead.value)}</div></div>) : <div className="empty">No leads</div>}</div>; })}</div>}
