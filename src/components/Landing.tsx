...
import { dashboardDB, jobPostingsStorageKey, type JobPosting } from "@/lib/dashboard";
...
export function Landing() {
  ...
  return <main className="landing-page">
    <header><div className="wrap nav"><div className="brand"><img src="/legacy-assets/embedded_asset_1.png" alt="JAAD Logistics" /></div><nav className="navlinks"><a href="#services">Services</a><a href="#coverage">Coverage</a><a href="#track">Track</a><a href="#about">About</a><a href="#careers">Careers</a><a href="#contact">Contact</a></nav><div className="navcta"><a href="#contact" className="btn small">Get a quote</a><a href="/admin/login" className="btn ghost small">Admin access</a><button className="btn ghost small" onClick={() => { setAuthMode("login"); setError(""); }}>Sign up / Login</button></div></div></header>
    ...
    <section id="careers"><div className="wrap"><div className="section-head"><div><div className="eyebrow">Careers</div><h2>Build the future of logistics with us.</h2></div><p>Join the team moving businesses across Nigeria and beyond.</p></div>{jobPostings.length ? jobPostings.map((posting, index) => <div className="job-card" key={`${posting.title}-${index}`}><div><h3>{posting.title}</h3><div className="job-meta">{posting.location} · {posting.type}</div><p>{posting.desc}</p></div><a className="btn" href={`mailto:info@jaadlogistics.com?subject=${encodeURIComponent(`Application: ${posting.title}`)}`}>Apply</a></div>) : <div className="careers-panel"><h3>No open roles right now.</h3><p>Send us your details and we&apos;ll keep you in mind for future opportunities.</p><a className="btn" href="mailto:info@jaadlogistics.com?subject=Career%20interest">Send your details</a></div>}</div></section>
